    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        else
        {
            ...
            if (!shouldTriggerFatal)
                combatRoom = (CombatRoom) null;
            else if (!attackCommand.Results.Any<DamageResult>((Func<DamageResult, bool>) (r => r.WasTargetKilled)))
            {
                combatRoom = (CombatRoom) null;
            }
            else
            {
               ...
               // sunkenTreasurePower sunkenTreasurePower = await PowerCmd.Apply<sunkenTreasurePower>(sunkenTreasure.Owner.Creature, 1M, sunkenTreasure.Owner.Creature, (CardModel) sunkenTreasure);
                combatRoom = (CombatRoom) null;
            }
        }
    }
